...
function validatelserID(uid, min, max) {
    var error = "";
...
function validatePassword(passid, min, max) {
    ...
    if (passid.value == "") {
        ...
        error = "Pas sword ต้องมีตัวเ ลขอย่างน้อย 1 ตัวโท";
        passid. style.background = 'Yellow' ;
        alert(error);
        passid.focus();
        return false;
...
function alphanumeric(uaddr) {
    ...
    if (uaddr.value.match(letters)) {
        ...
        alert( 'Address ต้องเป็นตัวอักษรหรือตัวเลข เท่านั้น');
        uaddr. focus();
        return false;
     }
}
